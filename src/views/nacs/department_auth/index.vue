<template>
  <div class="app-container">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-s-check"
            @click="handleAuth(scope.row)" v-hasPermi="['nacs:dept:auth']">授权管理</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <dept-auth-form ref="deptAuthForm" />
  </div>
</template>
...
import { getDictDatas, DICT_TYPE, getLineDatas } from '@/utils/dict'
import {listSimpleUsers} from "@/api/system/user";
import deptAuthForm from "@/views/nacs/department_auth/deptAuthForm.vue";
export default {
  name: "DepartmentAuth",
  components: { Treeselect, deptAuthForm },
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    /** 部门授权 */
    handleAuth(row) {
      this.$refs.deptAuthForm.show(row);
    },
    /** 查询部门列表 */
    getList() {
      this.loading = true;
