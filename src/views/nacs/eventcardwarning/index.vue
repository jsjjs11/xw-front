  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="门禁点名称" prop="dApName">
        <el-input v-model="queryParams.dApName" placeholder="请输入门禁点名称" clearable
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      ...
      <el-form-item label="备用字段1" prop="bkField1">
        <el-input v-model="queryParams.bkField1" placeholder="请输入备用字段1" clearable
                  @keyup.enter.native="handleQuery"/>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="openForm(undefined)"
                   v-hasPermi="['nacs:event-card-warning:create']">新增
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
                   :loading="exportLoading"
                   v-hasPermi="['nacs:event-card-warning:export']">导出
        </el-button>
      </el-col>
...
<!--      <el-table-column label="主键ID" align="center" prop="id"/>-->
      ...
      <el-table-column label="门禁点名称" align="center" prop="dApName"/>
      <el-table-column label="事件时间" align="center" prop="eventTime" width="180">
        <template v-slot="scope">
          <span>{{ parseTime(scope.row.eventTime) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="告警事件类型" align="center" prop="eventId"/>
      <el-table-column label="告警事件名称" align="center" prop="eventName"/>
      <el-table-column label="告警事件详情" align="center" prop="remark"/>
      <el-table-column label="创建时间" align="center" prop="createTime" width="180">
        <template v-slot="scope">
          <span>{{ parseTime(scope.row.createTime) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="备用字段1" align="center" prop="bkField1"/>
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="openForm(scope.row.id)"
                     v-hasPermi="['nacs:event-card-warning:update']">修改
          </el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['nacs:event-card-warning:delete']">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <pagination v-show="total > 0" :total="total" :page.sync="queryParams.pageNo" :limit.sync="queryParams.pageSize"
                @pagination="getList"/>
    <!-- 对话框(添加 / 修改) -->
    <EventCardWarningForm ref="formRef" @success="getList"/>
  </div>
</template>

<script>
import * as EventCardWarningApi from '@/api/nacs/eventcardwarning';
import EventCardWarningForm from './EventCardWarningForm.vue';

export default {
  name: "EventCardWarning",
  components: {
    EventCardWarningForm,
  },
  data() {
    return {
      // 遮罩层
