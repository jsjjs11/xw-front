/**
* @description: TODO
* @author: chj
* @date: 2025/6/23 09:22
* @version: v1.0
*/
<template>
  <div class="app" style="width:100%; height:100%;">
    <SvgContainer
      ref="svgContainer"
      :items="svgItems"
      width="100%"
      height="100%"
      @click="handleContainerClick"
      @click:element="handleElementClick"
      :viewBox="viewBox"
      :initialViewBox="initialViewBox"
    />
  </div>
</template>

<script>
import SvgContainer from './SvgContainer.vue'
export default {
  components: {
    SvgContainer
  },
  data() {
    return {
      svgItems: [],
      viewBox: { x: 0, y: 0, width: 0, height: 0 },
      initialViewBox: { x: 0, y: 0, width: 0, height: 0 },
      lineColors: {},
      //////////////////////
      linesPath: [],
      linesText: [],
      stationElements: [],
      stationText: [],
      stationImage: [],
      exStationImage: 'data:image/png;base64,'
    }
  },
  mounted() {
    this.logAllElements()
    this.$nextTick(() => {
      const el = this.$refs.svgContainer.$el;
      const w = el.clientWidth * 2
      const h = el.clientHeight * 2
      this.viewBox={ x: -w / 2, y: -h / 2, width: w, height: h }
      this.initialViewBox = { x: -w / 2, y: -h / 2, width: w, height: h }
    })
  },
  created() {
    this.generateLinesPath()
    this.generateStationEle()
    this.initSvgItems()
  },
  methods: {
    generateLinesPath() {
      const l = window.BMapSubWayData.subways.l
      const textArr = [];
      const pathArr = [];
      for (let i = 0; i < l.length; i++) {
        let { l_xmlattr, p } = l[i]
        var { lb, loop, uid } = l_xmlattr
        var dStr = ""; //地铁线路点
        var isLb = false; //是否圆润拐点
        for (let j = 0; j < p.length; j++) {
          let { x, y, lb, st, ex, rc } = p[j].p_xmlattr
          if (isLb) {
            isLb = false
            dStr += x + " " + y + " "
          } else {
            if (rc) {
              isLb = true
              dStr += "Q" + x + " " + y + " "
            } else {
              if (j == 0) {
                dStr += "M" + x + " " + y + " "
              } else {
                dStr += "L" + x + " " + y + " "
              }
              if (j == p.length - 1) {
                if (loop) {
                  dStr += "Z"
                }
              }
            }
          }
        } // end for
        var { lid, lb, lc, lbx, lby } = l_xmlattr
        pathArr.push({
          id: 'path-' + i + '--' + lid,
          type: 'path',
          d: dStr,
          stroke: '#' + lc.split('x')[1],
          fill: 'none',
          strokeWidth: 2
        })
        textArr.push({
          id: 'text-1' + i + '--' + lid,
          type: 'text',
          x: lbx - 8,
          y: lby + 15,
          fill: '#' + lc.split('x')[1],
          content: lb
        })
      } // end for
      // console.log(pathArr);
      this.linesPath = pathArr;
      //console.log(textArr);
      this.linesText = textArr;
    },
    generateStationEle() {
      const l = window.BMapSubWayData.subways.l
      const stationElementsArr = []
      const stationTextArr = []
      const imageArr = []
      let repeatStr = ''
      for (let i = 0; i < l.length; i++) {
        let { l_xmlattr, p } = l[i]
        for (let j = 0; j < p.length; j++) {
          let { x, y, rx, ry, lb, ex, rc, st, uid , sid} = p[j].p_xmlattr
          if (st && ('' !== sid)) {
            if (ex) { // 换乘站
              if (!repeatStr.includes(sid)) {
                imageArr.push({
                  type: 'image',
                  width: 20,
                  height: 20,
                  x: x - 10,
                  y: y - 10,
                  href: this.exStationImage
                })
              }
            } else { // 非换乘站
              stationElementsArr.push({
                id: 'circle-' + i + '--' + j,
                type: 'circle',
                cx: x,
                cy: y,
                r: 5,
                //fill: '#' + l_xmlattr.lc.split('x')[1],
                fill: '#fff',
                stroke: '#' + l_xmlattr.lc.split('x')[1],
                strokeWidth: 2
              })
            }
            if (!repeatStr.includes(sid)) {
              stationTextArr.push({
                id: 'text-1' + i + '--' + j,
                type: 'text',
                x: x + rx + 2,
                y: y + ry + 12,
                fill: '#' + l_xmlattr.lc.split('x')[1],
                content: lb
              })
              repeatStr += sid
            }
          } // end if st
        } // end for p
      } // end for l
      this.stationElements = stationElementsArr
      this.stationText = stationTextArr
      this.stationImage = imageArr
      // console.log(this.stationElements)
      // console.log(this.stationText)
      // console.log(this.stationImage)
    },
    resetViewBox() {
      this.$refs.svgContainer.resetViewBox()
    },
    initSvgItems() {
      this.svgItems = [...this.linesPath,...this.linesText,...this.stationElements,...this.stationText,...this.stationImage]
    },
    logAllElements() {
      const elements = this.$refs.svgContainer.getAllElements()
      console.log(`当前共有 ${elements.length} 个元素实例:`)
      /*elements.forEach(el => {
        console.log(`- ${el.itemData.type}: ${el.itemData.id}`)
      })*/
    },
    handleContainerClick(event) {
      console.log(event);
      console.log(`点击了容器: ${event.target.tagName}`)
    },
    handleElementClick(item, event) {
      console.log(`点击了元素: ${item.id} (${item.type})`)
    }
  }
}
</script>

<style scoped>
.app {
  font-family: Avenir, Helvetica, Arial, sans-serif;
  -webkit-font-smoothing: antialiased;
  -moz-osx-font-smoothing: grayscale;
  text-align: center;
  color: #ffffff;
  margin-top: 1px;
  width: 100%;
  height: 100%;
}
</style>
